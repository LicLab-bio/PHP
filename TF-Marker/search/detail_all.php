...
<div class="container">
    ...
    $sql = "select PMID,
        ...
        group_concat(DISTINCT Function SEPARATOR ', ') Function,group_concat(DISTINCT MarkerType SEPARATOR ', ') MarkerType,group_concat(DISTINCT DiseaseName SEPARATOR ', ') DiseaseName,
                ...
                <div class="col-lg-6">
                    <div class="box box-color-1">
                        <table class="table table-bordered table-striped table-hover">
                            ...
                            <?php if ($MarkerType=='Disease Marker') { ?>
                                <tr>
                                <td>Disease Name</td>
    ...
    <?php if($is_tf) { ?>
        <div class="panel panel-default">
            <div class="panel-heading">
                <h3>
                    <i class="ri-folder-info-line"></i> The relationship between TF- Super Enhancer(SE)- Marker gene
                </h3>
            </div>
            <div class="panel-body">
                ...
                <div class="box box-color-1">
                    <br>
                    <table class="table table-hover table-bordered table-condensed" id="SE">
                        <thead>
                        <th>TF</th>
                        <th>SEID</th>
                        <th>Gene</th>
                      ...
                      <th>Sample_Name</th>
                        ...
                        <tbody>
                        <?php
                        $sql_str = "where (Gene='" . implode("' and TF='" . $GeneName . "' or Gene='",$gene_names)."' and TF='" . $GeneName . "')";

                        $SE_sql = "SELECT *
                    from `TF-Marker`.SEanalysis_Final
                    $sql_str and Tissue_Type='".$TissueType."'
                        union 
                        SELECT *
                    from `TF-Marker`.SEanalysis_Final
                    $sql_str and Tissue_Type!='".$TissueType."'";

                        $SE_res = mysqli_query($conn,  $SE_sql);


                        while ($row = mysqli_fetch_assoc($SE_res)) {
                                echo "<tr>";
                                echo "<td>{$row["TF"]}</td>";
                                echo "<td><a href='http://licpathway.net/SEanalysis/browseSearch_result.do/?sampleId={$rows["SampleID"]}'>{$row["SEID"]}</a></td>";
                                echo "<td>{$row["Gene"]}</td>";
                                echo "<td><a href='http://licpathway.net/SEanalysis/browseSearch_result.do/?sampleId={$rows["SampleID"]}'>{$row["SampleID"]}</a></td>";
                                echo "<td>{$row["Sample_Name"]}</td>";
                                echo "<td>{$row["Biosample_Type"]}</td>";
                                echo "<td>{$row["Tissue_Type"]}</td>";
                               echo "</tr>";
                            }
...
<script>
    ...
    Highcharts.chart('gene_distribution', {
        ...
        title: {
            text: 'Frequency of <?php echo $GeneName?> <span class="glyphicon glyphicon-question-sign">'
        },
        xAxis: {
