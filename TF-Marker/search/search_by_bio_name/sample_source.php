...
$search = $search."order by sample_source";

$search_sql="SELECT distinct sample_source from sample_dataset $search";
...
while($row = mysql_fetch_assoc($search_result)){
	echo "<option value='{$row['sample_source']}'>{$row['sample_source']}</option>\n";
}
?>